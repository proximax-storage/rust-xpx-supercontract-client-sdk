...
impl BlockchainInterface {
    ...
    pub unsafe fn get_block_hash(&mut self) -> String {
        let ret = get_block_hash(self.buffer.as_mut_ptr() as u64);
        return String::from_utf8_unchecked(self.buffer[..ret as usize].to_vec());
    }

    pub unsafe fn get_transaction_hash(&mut self) -> String {
        let ret = get_transaction_hash(self.buffer.as_mut_ptr() as u64);
        return String::from_utf8_unchecked(self.buffer[..ret as usize].to_vec());
    }

    pub unsafe fn get_caller_public_key(&mut self) -> String {
        let ret = get_caller_public_key(self.buffer.as_mut_ptr() as u64);
        return String::from_utf8_unchecked(self.buffer[..ret as usize].to_vec());
    }
    ...
    pub unsafe fn get_response_transaction_hash(&mut self, hash: String) -> Vec<String> {
        let hash = hash.as_bytes();
        let mut buf = Vec::new();
        get_response_transaction_hash(hash.as_ptr() as u64, buf.as_mut_ptr() as u64); // The return value is the total length of the buffer written, so I'm not using it
    ...
    pub unsafe fn get_transaction_content(&mut self, hash: String) -> String {
        let hash = hash.as_bytes();
        let ret = get_transaction_content(hash.as_ptr() as u64, self.buffer.as_mut_ptr() as u64);
        return String::from_utf8_unchecked(self.buffer[..ret as usize].to_vec());
    }
    ...
    pub unsafe fn get_call_params(&mut self) -> Result<HashMap<String, String>> {
        let len = get_call_params(self.buffer.as_mut_ptr() as u64);
